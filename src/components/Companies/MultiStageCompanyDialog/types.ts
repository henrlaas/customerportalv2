...
export const companyFormSchema = z.object({
  ...
  website: z.string().url({ message: 'Please enter a valid URL' }).min(1, { message: 'Website is required' }),
  phone: z.string().min(1, { message: 'Phone number is required' }),
  invoice_email: z.string().email({ message: 'Please enter a valid email' }).min(1, { message: 'Invoice email is required' }),
  ...
  mrr: z.coerce.number().min(0, { message: 'MRR must be a positive number' }),
});
